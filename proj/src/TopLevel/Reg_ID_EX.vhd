...
-- injecting zeros (NOPS)
  ...
  s_RS1_Addr <= (others => '0') when i_FLUSH = '1' else i_RS1_Addr;
  s_RS2_Addr <= (others => '0') when i_FLUSH = '1' else i_RS2_Addr;
  ...
  o_SHALT <= q_SHALT(0);
  o_BR <= q_BR(0);
  o_RS1_Addr <= q_RS1_Addr;
  o_RS2_Addr <= q_RS2_Addr;
  o_MemRead(0)  <= q_MemRead(0);
