...
architecture structure of RISCV_Processor is
  ...
  signal s_MemRead        : std_logic_vector(0 downto 0); -- Calculated in ID stage
  signal s_MemRead_ID_EX  : std_logic_vector(0 downto 0); -- Output of ID/EX Register
  ...
  -- signal fixed for lui
  signal s_RS1_Addr_Fixed : std_logic_vector(4 downto 0);
...
component HDU is 
	port(
		i_ID_EX_MemRead	: in std_logic_vector(0 downto 0); -- checks if instruction is a load
...
begin
  ...
  FL: FetchLogic
	...
	ALUo => s_ALUOut,
	PCsrc => s_PCsrc,
	jump_target => s_ALUOut,
	pc_branch => s_PC_ID_EX,
	PC_Write => s_PC_Write,
	PCP4 => s_NextInstAddr,
	currPC => s_PCOut
	);
  ...
  IF_ID: Reg_IF_ID
	...
	-- Logic to determine if a Branch/Jump is actually happening
		  s_Branch_taken <= 
		  '1' when s_PCsrc = "01" else -- Handle Conditional Branch
                  '1' when s_PCsrc = "10" else -- Handle JAL
                  '1' when s_PCsrc = "11" else -- Handle JALR
                  '0';
  ...
  s_MemRead(0) <= '1' when s_WBSel = "00" else '0';
  -- LUI Fix: Force RS1 to 0 if Opcode is LUI (0110111), otherwise read standard bits
  s_RS1_Addr_Fixed <= (others => '0') when s_INST_IF_ID(6 downto 0) = "0110111" else 
                    s_INST_IF_ID(19 downto 15);

  RF: RegisterFile
	port map(
		i_RD1 => s_RegWrAddr,
		i_RS1 => s_RS1_Addr_Fixed,
...
-- forwarding mux for rs2 or Bmux
  ...
  begin
  	...
  	case s_INST_ID_EX(6 downto 0) is
  		...
  		when "1100011" => -- BRANCH OPCODE
		        -- We trust s_Branch because it combines:
		        -- 1. Control Signal (It is a branch instruction)
		        -- 2. Comparator Output (The condition is met)
		        
		        if (s_Branch = '1') then
		            s_PCsrc <= "01"; -- Take the branch!
		        else
		            s_PCsrc <= "00"; -- Fall through
		        end if;
  		when others =>
  			s_PCsrc <= "00";
  	end case;
  ...
  FU: Forwarding_Unit
  	port map(
  		i_ID_EX_RS1 => s_RS1_Addr_ID_EX,
  		i_ID_EX_RS2 => s_RS2_Addr_ID_EX,
  ...
  --WRDATAMUX: mux4t1_N
	...
	--);
	with s_WBSel_MEM_WB select
    		s_RegWrData <= s_LoadData_Ext    when "00", -- Select 00: Memory Data
		           s_ALUOut_MEM_WB   when "01", -- Select 01: ALU Result
		           s_PCP4_MEM_WB     when "10", -- Select 10: PC+4
		           s_ImmOut_MEM_WB   when "11", -- Select 11: Immediate
